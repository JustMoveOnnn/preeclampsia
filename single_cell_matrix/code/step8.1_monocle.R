...
library(plyr)
library(ggplot2)
pbmc <- readRDS("pbmc20191127notransi.RDS")
# ****** select cluster you want **** #
######### ***************** IMMU ***************** ###############
# exp_data1=subset(x=pbmc,subset = annoClust=="NK1" & loc1_disease=="deci_NM" )
# exp_data2=subset(x=pbmc,subset = annoClust=="NK2" & loc1_disease=="deci_NM" )
# exp_data3=subset(x=pbmc,subset = annoClust=="T" & loc1_disease=="deci_NM" )
# exp_data4=subset(x=pbmc,subset = annoClust=="APC" & loc1_disease=="deci_NM" )
######### ***************** TRO + dSC + transi ***************** ###############

exp_data2=subset(x=pbmc,subset = annoClust=="EVT1" & loc1_disease=="pla_PE" )
exp_data3=subset(x=pbmc,subset = annoClust=="EVT2" & loc1_disease=="pla_PE" )
exp_data4=subset(x=pbmc,subset = annoClust=="EVT3" & loc1_disease=="pla_PE" )
exp_data5=subset(x=pbmc,subset = annoClust=="STB1" & loc1_disease=="pla_PE" )
exp_data6=subset(x=pbmc,subset = annoClust=="STB2" & loc1_disease=="pla_PE" )
exp_data1=subset(x=pbmc,subset = annoClust=="CTB" & loc1_disease=="pla_PE" )
exp_data56 <- merge(x=exp_data5, y=exp_data6)

exp_data12 <- merge(x=exp_data1, y=exp_data2)
exp_data34 <- merge(x=exp_data3, y=exp_data4)
exp_data1234 <- merge(x=exp_data12, y=exp_data34)


exp_data <- merge(x=exp_data1234, y=exp_data56)
...
head(HSMM_myo@featureData@data)
pdf('./pla_PE/plot_ordering_genes.pdf')
...
pdf('./pla_PE/plot_cell_trajectory.State.pdf')
plot_cell_trajectory(HSMM_myo, color_by = "State")
dev.off()
pdf('./pla_PE/plot_cell_trajectory.Pseudotime.pdf')
plot_cell_trajectory(HSMM_myo, color_by = "Pseudotime")
dev.off()

pdf('./pla_PE/plot_cell_trajectory.cellType.pdf')
plot_cell_trajectory(HSMM_myo, color_by = "cellType")
dev.off()

pdf('./pla_PE/fenkaihua.pdf')
plot_cell_trajectory(HSMM_myo, color_by = "cellType") + facet_wrap(~cellType,nrow=1)
dev.off()


# show_genes <- row.names(subset(fData(HSMM_myo), gene_short_name %in% c("EVT")))
# pdf('./deci_PE/plot_genes_jitter.pdf')
...
gene_short_name %in% c("NKG7","NCAM1", "GZMB","PRF1","CD74","MS4A7","CD3D","IL7R")))
pdf('./deci_PE/plot_genes_jitter.pdf')
